...
if (!pass) {
	console.error("usage: node login.mjs <user> <pass>");
	process.exit(1);
}
...
if (step.type !== "create_entry")
	throw new Error(`login failed: ${JSON.stringify(step.errors || step)}`);
const tok = await post(
	"/auth/token",
	{ grant_type: "authorization_code", code: step.result, client_id: CID },
	true,
);
await import("node:fs").then((fs) =>
	fs.writeFileSync(
		new URL("./.ha-token.json", import.meta.url),
		JSON.stringify({ refresh_token: tok.refresh_token }, null, 2),
	),
);
