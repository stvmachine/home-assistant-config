...
const EXCLUDE = [
  'secrets.yaml', '.cloud', 'deps', 'tts', 'image', 'backups',
  '.storage/auth', '.storage/auth_provider.homeassistant', '.storage/http.auth',
  '.storage/application_credentials', '.storage/cloud', '.storage/core.restore_state',
  '.storage/bluetooth.passive_update_processor', '.storage/onboarding', '.storage/backup',
];
...
async function accessToken() {
  ...
  const r = await fetch(`http://${HOST}/auth/token`, {
    ...
    body: new URLSearchParams({ grant_type: 'refresh_token', refresh_token, client_id: `http://${HOST}/` }),
  });
...
async function main() {
  ...
  await ha.send({
    ...
    include_addons: [], include_all_addons: false, include_database: false,
    include_folders: [], include_homeassistant: true,
    name: 'git-snapshot',
  });

  let backup;
  for (let i = 0; i < 60 && !backup; i++) {          // config-only backups take seconds
    await sleep(2000);
  ...
  const dl = await fetch(`http://${HOST}/api/backup/download/${backup.backup_id}?agent_id=hassio.local`,
  ...
  const mine = (await ha.send({ type: 'backup/info' })).backups
    .filter((b) => b.name === 'git-snapshot').sort((a, b) => b.date.localeCompare(a.date));
  for (const old of mine.slice(KEEP)) await ha.send({ type: 'backup/delete', backup_id: old.backup_id });
  for (const f of fs.readdirSync(`${REPO}/backups`).filter((f) => f.endsWith('.tar')).sort().reverse().slice(KEEP))
  ...
  sh('tar', ['xzf', `${tmp}/homeassistant.tar.gz`, '-C', `${REPO}/config`, '--strip-components=1', 'data']);
  ...
  for (const f of fs.readdirSync(`${REPO}/config/.storage`).filter((f) => /\.(pem|key|crt|p12)$/.test(f)))
  ...
  for (const f of fs.readdirSync(`${REPO}/config/.storage`)) {
    ...
    try { fs.writeFileSync(p, JSON.stringify(JSON.parse(fs.readFileSync(p, 'utf8')), null, 2) + '\n'); }
